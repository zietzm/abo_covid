...
header-includes: '<!--
  ...
  <link rel="alternate" type="text/html" href="https://zietzm.github.io/abo_covid/v/d6d0bc9fb2513cbd38f87cf600ba580bff145ae4/" />

  <meta name="manubot_html_url_versioned" content="https://zietzm.github.io/abo_covid/v/d6d0bc9fb2513cbd38f87cf600ba580bff145ae4/" />

  <meta name="manubot_pdf_url_versioned" content="https://zietzm.github.io/abo_covid/v/d6d0bc9fb2513cbd38f87cf600ba580bff145ae4/manuscript.pdf" />
...
<small><em>
This manuscript
([permalink](https://zietzm.github.io/abo_covid/v/d6d0bc9fb2513cbd38f87cf600ba580bff145ae4/))
was automatically generated
from [zietzm/abo_covid@d6d0bc9](https://github.com/zietzm/abo_covid/tree/d6d0bc9fb2513cbd38f87cf600ba580bff145ae4)
on April 7, 2020.
</em></small>
...
While our results are based on preliminary observational data collected during the care of patients and should not be used to guide clinical practice, we find new evidence of associations between B and Rh blood groups and COVID-19 and further evidence of recently-discovered associations between blood group and SARS-CoV-2 infection status.
...
We performed a multivariate analysis of our results to evaluate potential confounding due to risk factors, and we meta-analyzed our results in combination with data from China.
...
Throughout our analysis, individuals with a single positive SARS-CoV-2 test are considered COV+, even if they had previous or subsequent negative tests.
...
We first determined blood groups for SARS-CoV-2-tested individuals using laboratory measurements recorded in the NYP EHR system.
...
Finally, we compared the blood group distributions between all individuals tested for SARS-CoV-2 with the general population at NYP, finding insufficient evidence to conclude that tested individuals are not drawn from the general population at random (Chi-squared 3.8896, p=0.2736).
...
We find that the significant associations we report are not the result of confounding due to several known risk factors we considered.
For risk factors, we considered age, sex, overweight status, diabetes mellitus, hypertension, pulmonary diseases, and cardiovascular diseases.
...
These results indicated potential multiple collinearity between risk factors and blood groups.

In the next analysis, though, we found that blood groups provide significant additional information on outcomes beyond risk factors.
...
To do so, we inspected individual blood group coefficients between nested (outcome ~ blood group) and full (outcome ~ blood group) logistic regression models.
For ABO blood groups, we found that coefficients only marginally changed, with some coefficients being more extreme (greater in magnitude and more significant) in full models than in nested models (Table @tbl:bg_coef).
...
These results provide no evidence for conditional independence between outcomes and blood group given risk factors, meaning we have no evidence that the significant effects of blood group we found are modulated by risk factors.
...
In addition to the results Zhao et al. reported, we find associations for B blood groups as well.
...
Our results from NYP identified significant associations between A, AB, and O blood groups.
However, furher stratifying by Rh resulted in significant associations for A+ and O+ only.
...
However, the increased sample size afforded by adding NYP data allowed the B blood group association to be declared significant at the 5% level.
...
Using data from NYP, we found the odds of COVID-19 positive vs negative test results were increased in blood groups A and decreased in blood groups O, consistent with previous results from Wuhan and Shenzhen.
...
AB blood groups were not included because they are mutually exclusive with A, B, and O blood groups. Full data are available [on GitHub](https://github.com/zietzm/abo_covid_analysis). {#tbl:bg_coef}
